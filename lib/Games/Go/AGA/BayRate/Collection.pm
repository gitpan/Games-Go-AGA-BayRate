...
use Math::GSL::Errno ( ':all' ); # for various constants like $GSL_CONTINUE
use Math::GSL::Const ( ':all' ); # for various constants like $M_PI
use Math::GSL::Multimin ( ':all' ); # for f(df)minimizer functions
...
sub print_vector {
    ...
    if (not $count or $count <= 0) {
        $count = Games::Go::AGA::BayRate::GSL::Multimin::my_vector_size($raw_v);    # what a hack!
    }
    my $ii;
    for ($ii = 0; $ii < $count; $ii ++) {
        if ($ii % 10 == 0) { printf("%3d:", $ii); }
        printf(" % .24g", gsl_vector_get($raw_v, $ii));
        if ($ii % 10 == 9) { print("\n"); }
    }
    if ($ii % 10 != 0) { print("\n"); }
}
...
sub calc_ratings_f {
    ...
    # minimizer 'state'
    my $state = Games::Go::AGA::BayRate::GSL::Multimin::my_fminimizer_set (
...
#       }
    ...
    # minimizer 'state'
    my $state = Games::Go::AGA::BayRate::GSL::Multimin::my_fdfminimizer_set (
